					...
					<div style="height: 50em; overflow: scroll;">
						<c:forEach items="${inventories}" var="inventory"
							...
							<div class="card card-block" style="width: 100%;">
								<h3 class="card-title text-primary"><strong>${inventory.getProduct().getName()}</strong></h3>
								<h4 class="card-text text-muted">Stock left: <strong><span class="text-danger">${inventory.getStock()}</span></strong></h4>
								<h5 class="card-text text-muted">Last updated: <strong><span class="text-success">${inventory.getDateUpdated()}</span></strong></h5>
								<h5 class="ccard-text text-muted">Updated by: <strong><span class="text-success">${inventory.getUpdater().getFirstName()} &nbsp; ${inventory.getUpdater().getLastName()}</span></strong></h5>
								<a href="updateInventory?name=${inventory.getProduct().getName()}" class="btn btn-success">Re-stock</a>
							</div>
						</c:forEach>
					</div>
				</c:when>
